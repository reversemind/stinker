...
module Stinker
  class Markup
    ...
    #
    # page - The Stinker::Page.
    #
    # Returns a new Stinker::Markup object, ready for rendering.
    def initialize(page)
      @site    = page.site
      @name    = page.filename
      @data    = page.text_data
    ...
    # Render the content with Stinker wiki syntax on top of the file's own
    # markup language.
    #
    ...
    def process_image_tag(tag)
      ...
      path  = if file = find_file(name)
        ::File.join @site.base_path, file.path
    ...
    def process_file_link_tag(tag)
      ...
      if name && path && file
        %{<a href="#{::File.join @site.base_path, file.path}">#{name}</a>}
      elsif name && path
        %{<a href="#{path}">#{name}</a>}
      else
        nil
    ...
    def process_page_link_tag(tag)
      ...
      cname = @site.page_class.cname(page_name || name)
      ...
      if name =~ %r{^https?://} && page_name.nil?
        ...
        if page
          link_name = @site.page_class.cname(page.name)
          presence  = "present"
        end
        link = ::File.join(@site.base_path, CGI.escape(link_name))
    ...
    #
    # Returns the Stinker::File or nil if none was found.
    def find_file(name)
      if name =~ /^\//
        @site.file(name[1..-1], @version)
      else
        path = @dir == '.' ? name : ::File.join(@dir, name)
        @site.file(path, @version)
      end
    end
    ...
    #
    # Returns a Stinker::Page instance if a page is found, or an Array of
    # [Stinker::Page, String extra] if a page without the extra anchor data
    # is found.
    def find_page_from_name(cname)
      if page = @site.page(cname)
        return page
      end
      if pos = cname.index('#')
        [@site.page(cname[0...pos]), cname[pos..-1]]
      end
    end
    ...
    def process_code(data)
      ...
      highlighted = begin
        blocks.size.zero? ? [] : Stinker::Albino.colorize(blocks)
